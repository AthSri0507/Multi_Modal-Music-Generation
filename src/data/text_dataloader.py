"""PyTorch dataset and dataloader utilities for text embeddings."""
...
class HDF5TextEmbeddingDataset(Dataset):
    """Dataset backed by split groups from embeddings HDF5 file."""

    def __init__(
        self,
        h5_path: str,
        split: str,
        label_encoder: Optional[TextLabelEncoder] = None,
    ) -> None:
        ...
        with h5py.File(self.h5_path, "r") as h5f:
            ...
            grp = h5f[split]
            self.size = int(grp["embeddings"].shape[0])
        ...
        if raw_labels and label_encoder is None:
            ...
            label_encoder = TextLabelEncoder(label_to_id={label: i for i, label in enumerate(unique)})
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        with h5py.File(self.h5_path, "r") as h5f:
            emb = h5f[self.split]["embeddings"][idx]
...
def build_text_dataloader(
    dataset: HDF5TextEmbeddingDataset,
    batch_size: int = 64,
    shuffle: bool = True,
    num_workers: int = 0,
