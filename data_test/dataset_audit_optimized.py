...
# Load Spotify (900k rows - can be slow)
print("Loading Spotify data...", file=sys.stderr)
sys.stderr.flush()
spotify = pd.read_csv(spotify_path, low_memory=False)
print(f"  ✓ Loaded {len(spotify)} rows, {len(spotify.columns)} cols", file=sys.stderr)
sys.stderr.flush()

# Load Genius
print("Loading Genius data...", file=sys.stderr)
sys.stderr.flush()
genius = pd.read_csv(genius_path, low_memory=False)
print(f"  ✓ Loaded {len(genius)} rows, {len(genius.columns)} cols", file=sys.stderr)
sys.stderr.flush()
...
print("Computing missing rates...", file=sys.stderr)
sys.stderr.flush()
...
if s_track and s_artist:
    print("Computing Spotify dedup...", file=sys.stderr)
    sys.stderr.flush()
...
if g_track and g_artist:
    print("Computing Genius dedup...", file=sys.stderr)
    sys.stderr.flush()
...
# High-confidence merged subset
if s_track and s_artist and g_track and g_artist:
    print("Computing merge quality...", file=sys.stderr)
    sys.stderr.flush()
    ...
    print(f"  Spotify after dedup: {len(s)}", file=sys.stderr)
    sys.stderr.flush()
    ...
    print(f"  Genius after dedup: {len(g)}", file=sys.stderr)
    sys.stderr.flush()
